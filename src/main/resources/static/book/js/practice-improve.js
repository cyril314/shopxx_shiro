...
//		 }, false);
	...
	bindSubmitQuestion : function bindSubmitQuestion(){
		$("#submit-q-btn").click(function(){
			...
			{
				var realAnswer = examing.getRealAnswer();
...
//			modal.showProgress();
			...
			$.ajax({
				url: "answer",
				type: "POST",
				data: {myAnswer: data.myAnswer,answer:data.answer,questionId:data.questionId},
				dataType: "json",
				cache: false,
	...
	getRealAnswer :function()
	{
		var thisquestion  = $(".question:visible");
		var answer;
		
		if(thisquestion.hasClass("qt-singlechoice") || $(thisquestion).hasClass("qt-multiplechoice") || $(thisquestion).hasClass("qt-trueorfalse"))
		{
			var optionSpan = $(thisquestion).find("span.answer_option");
			...
			{
				var tm_answer = "";
				for(var l = 0 ; l < optionSpan.length; l++)
				{
					if (l != 0)
					{
						tm_answer += "|"
					}
					tm_answer = tm_answer + $(optionSpan[l]).attr("optionId");
				}
				answer = tm_answer;
			}
		} 
...
var modal = {
	prepare : function prepare() {
		$(".content").append("<div id=\"loading-progress\" style=\"display:none;\"><div id=\"loading-content\"> <h2>正在提交您的答案</h2><img class=\"loading-gif\" src=\"../../static../../images/loading.gif\"/><div> </div>");
