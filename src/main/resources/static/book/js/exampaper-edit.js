...
var examing = {
            ...
            if (checkboxs.length == 0) {
                ...
                $.ajax({
                    ...
                    success: function (message) {
                        if (message.type == "success") {
                            $("#exampaper-body").append(message.content);
                        } else {
    ...
    getType: function getType(input) {
        ...
        if (m) {
            ...
            if (m[1]) { return 'float'; } else { return 'int'; }
        }
        return 'string';
    },

    bindDialog: function bindDialog() {
        var $iframe = $("#qt-selector-iframe");
        var $button = $("#add-more-qt-to-paper");

        $button.click(function () {
            var $questions = $("span.question-id");
            var args = "";
            $questions.each(function () {
                args += $(this).text() + "|";
            });
            $iframe.attr("src", "choice?questions=" + args);
        });
    }
};