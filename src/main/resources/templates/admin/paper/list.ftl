...
<body>
	<div class="path">
		<a href="${base!}/admin/common/index">${message("shop.path.index")}</a> &raquo; 试卷列表 <span>(${message("shop.page.total", page.total)})</span>
	</div>
	<form id="listForm" action="list" method="get">
		...
		<table id="listTable" class="list">
			...
			[#list page.content as paper]
				<tr>
					<td>
						<input type="checkbox" name="ids" value="${paper.id}" />
					</td>
					<td>
						<span title="${paper.name}">${abbreviate(paper.name, 50, "...")}</span>
					</td>
					<td>
						${paper.costTime}
					</td>
					...
					<td>
						[#if paper.status == 'edit']
						<a href="edit?id=${paper.id}">[${message("shop.common.edit")}]</a>
						<a href="question?id=${paper.id}">[试题维护]</a>
						[/#if]
					</td>
				</tr>
			[/#list]
